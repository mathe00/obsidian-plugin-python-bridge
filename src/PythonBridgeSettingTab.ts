	...
	display(): void {
		...
		new Setting(containerEl)
			...
			.addDropdown((dropdown) => {
				...
				dropdown.addOption(
					"auto",
					t("SETTINGS_LANGUAGE_AUTO") || "Automatic",
				);
					...
					.onClick(async () => {
						...
						if (!scriptsFolder) {
							if (this.plugin.settings.pythonScriptsFolder) {
								...
								new Notice(
									t("NOTICE_SCRIPTS_FOLDER_INVALID"),
									5000,
								);
							}
							return;
						...
						button
							...
							.setButtonText(
								t(
									"SETTINGS_REFRESH_DEFINITIONS_BUTTON_REFRESHING",
								),
							);
						...
						// FIX: Catch block was inside the try block before
							...
							if (button.buttonEl && button.disabled) {
								button
									...
									.setButtonText(
										t(
											"SETTINGS_REFRESH_DEFINITIONS_BUTTON_TEXT",
										),
									);
							}
						}
		...
		try {
			scriptFiles = fs
				...
				.filter(
					(f) =>
						...
						!f.startsWith(".") &&
						f !== PYTHON_LIBRARY_FILENAME, // Exclude library
				);
		} catch (error) {
			this.plugin.logError(
			...
			containerEl.createEl("p", {
				text: t("NOTICE_SCRIPTS_FOLDER_READ_ERROR_PREFIX") +
		...
		if (scriptFiles.length === 0) {
			containerEl.createEl("p", {
				text: t("NOTICE_NO_SCRIPTS_FOUND"), // Use the existing "no scripts" notice
			});
		} else {
			...
			for (const scriptFilename of scriptFiles) {
				...
				if (scriptDefs && scriptDefs.length > 0) {
					...
					for (const settingDef of scriptDefs) {
						...
						const currentValue = scriptValues.hasOwnProperty(
							settingDef.key,
						)
