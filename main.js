    ...
    return __awaiter(this, arguments, void 0, function* (plugin, scriptPath, context = "manual") {
        ...
        try {
            yield new Promise((resolve, reject) => {
                ...
                let stderrOutput = "";
                (_a = pythonProcess.stderr) === null || _a === void 0 ? void 0 : _a.on("data", (data) => { const msg = data.toString(); stderrOutput += msg; console.error(`[stderr ${scriptFilename}]: ${msg.trim()}`); plugin.logError(`[stderr ${scriptFilename}]: ${msg.trim()}`); });
                (_b = pythonProcess.stdout) === null || _b === void 0 ? void 0 : _b.on("data", (data) => { const msg = data.toString(); console.log(`[stdout ${scriptFilename}]: ${msg.trim()}`); plugin.logDebug(`[stdout ${scriptFilename}]: ${msg.trim()}`); });
        ...
        try {
            yield new Promise((resolve, reject) => {
                ...
                (_a = pythonProcess.stderr) === null || _a === void 0 ? void 0 : _a.on("data", (data) => {
                    ...
                    console.error(`[stderr EVENT ${scriptFilename}]: ${data.toString().trim()}`);
                    plugin.logError(`[stderr EVENT ${scriptFilename}]: ${data.toString().trim()}`);
                });
                (_b = pythonProcess.stdout) === null || _b === void 0 ? void 0 : _b.on("data", (data) => {
                    console.log(`[stdout EVENT ${scriptFilename}]: ${data.toString().trim()}`);
...
function getActiveNoteAbsolutePath(plugin) {
    ...
    if (!file || !vaultPath)
        return null;
    return obsidian.normalizePath(path__namespace.join(vaultPath, file.path)); // Use normalizePath for consistency across OS
}
/**
    ...
    return __awaiter(this, arguments, void 0, function* (plugin, relativePath, permanently = false) {
        ...
        try {
            if (permanently) {
                ...
                yield plugin.app.vault.delete(fileOrFolder, true);
            } // Force = true for permanent
            else {
                yield plugin.app.vault.trash(fileOrFolder, true);
            } // system = true
            ...
            switch (action) {
                ...
                case "modify_note_content": // Kept for backward compatibility
                    if (typeof (payload === null || payload === void 0 ? void 0 : payload.filePath) !== "string" || typeof (payload === null || payload === void 0 ? void 0 : payload.content) !== "string")
                        return { status: "error", error: "Invalid payload: 'filePath' (absolute path string) and 'content' (string) required." }; // Expecting absolute path here
                    try {
                        ...
                        const normalizedFilePath = obsidian.normalizePath(payload.filePath);
                        if (!normalizedFilePath.startsWith(vaultAbsPath))
                            throw new Error("Path is outside the current vault.");
                        const relativePath = obsidian.normalizePath(path__namespace.relative(vaultAbsPath, normalizedFilePath));
                ...
                case "modify_note_content_by_path": // Preferred action
                    if (typeof (payload === null || payload === void 0 ? void 0 : payload.path) !== "string" || typeof (payload === null || payload === void 0 ? void 0 : payload.content) !== "string")
                        return { status: "error", error: "Invalid payload: 'path' (relative vault path string) and 'content' (string) required." }; // Expecting relative path here
                    try {
