...
export function getScriptsFolderPath(plugin: ObsidianPythonBridge): string {
	...
	if (!pythonScriptsFolder || typeof pythonScriptsFolder !== "string") {
		plugin.logWarn(
			"Python scripts folder path is not configured in settings.",
		);
		return "";
	}
...
export async function discoverScriptSettings(
	...
	return new Promise((resolve) => {
		...
		plugin.logDebug(
			`Running discovery command: ${plugin.pythonExecutable} ${args.join(" ")}`,
		);

		const scriptDir = path.dirname(scriptAbsolutePath);
		const currentPYTHONPATH = process.env.PYTHONPATH;
		// Prepend script's directory to PYTHONPATH for relative imports within the script
		...
		const env = {
			...
			// Pass other relevant env vars if needed for discovery? Usually not.
		};

		plugin.logDebug(`Executing discovery with PYTHONPATH: ${newPYTHONPATH} and cwd: ${scriptDir}`);

		const pythonProcess = spawn(plugin.pythonExecutable!, args, {
			timeout: discoveryTimeoutMs,
			cwd: scriptDir, // Set working directory to the script's folder
		...
		pythonProcess.on("close", (code, signal) => {
			...
			try {
				...
				if (!trimmedStdout) {
					...
					resolve([]); // Success, but no settings defined
					return;
				}
		...
		try {
			...
			if (definitions !== null) {
				...
				if (
					...
					changesMade = true;
					plugin.logDebug(`Definitions updated for ${relativePath}`);
				}
			} else {
				if (
					...
					newDefinitions[relativePath] =
						plugin.settings.scriptSettingsDefinitions[
							relativePath
						];
					plugin.logWarn(
						`Failed to discover settings for ${relativePath}, keeping cached version.`,
					);
				} else {
					plugin.logWarn(
						`Failed to discover settings for ${relativePath}, no cached version available.`,
					);
				}
			}
		} catch (error) {
			plugin.logError(
				`Unexpected error during settings discovery for ${file}:`,
				error,
			);
			if (
				plugin.settings.scriptSettingsDefinitions.hasOwnProperty(
...
export async function runPythonScript(
	plugin: ObsidianPythonBridge,
	scriptPath: string,
	context: string = "manual", // Add context for logging
) {
	if (!plugin.pythonExecutable) {
		plugin.logError(
	...
	if (
		...
		if (context === "manual") {
			new Notice(
				t("NOTICE_SCRIPT_DISABLED").replace(
					"{scriptName}",
					scriptFilename,
				),
			);
	...
	if (relativePath) {
		plugin.logDebug(
			`Setting OBSIDIAN_SCRIPT_RELATIVE_PATH=${relativePath}`,
		);
	}
	...
	try {
		await new Promise<void>((resolve, reject) => {
			...
			const pythonProcess = spawn(pythonCmd, fullArgs, {
				env,
				cwd: scriptDir, // Set CWD to script's directory
			});
			...
			pythonProcess.stderr?.on("data", (data) => {
				...
				stderrOutput += msg;
				console.error(`[stderr ${scriptFilename}]: ${msg.trim()}`);
				plugin.logError(`[stderr ${scriptFilename}]: ${msg.trim()}`);
			});

			pythonProcess.stdout?.on("data", (data) => {
				const msg = data.toString();
				console.log(`[stdout ${scriptFilename}]: ${msg.trim()}`);
			...
			pythonProcess.on("close", (code) => {
				...
				if (code !== 0 && code !== null) {
					...
					reject(
						new Error(`Script exited with non-zero code: ${code}`),
					);
				} else {
					resolve(); // Success
				}
...
export async function chooseAndRunPythonScript(
	...
	new ScriptSelectionModal(plugin.app, scriptChoices, (selectedPath) => {
		if (selectedPath) {
			...
			runPythonScript(plugin, selectedPath, "manual"); // Don't await, run in background
		} else {
...
export async function runAllPythonScripts(
	...
	const activeScriptsToRun = pythonFiles.filter((file) => {
		...
		return plugin.settings.scriptActivationStatus[relativePath] !== false; // Check activation
	});

	if (activeScriptsToRun.length === 0) {
		new Notice(t("NOTICE_NO_SCRIPTS_FOUND"), 5000); // Or a more specific message like "No active scripts found"
	...
	for (const file of activeScriptsToRun) {
		...
		await runPythonScript(plugin, scriptPath, "manual"); // Context is still manual trigger
	}
...
export function runAutoStartScripts(plugin: ObsidianPythonBridge): void {
			...
			try {
				if (
					...
					if (delayMs > 0) {
						...
						setTimeout(() => {
							// Re-check activation status inside timeout
							if (
								...
								plugin.logWarn(
									`Skipping delayed auto-start for ${relativePath}: Script was disabled during delay.`,
								);
							}
						}, delayMs);
					} else {
	...
	if (scriptsRunCount > 0) {
		...
		plugin.logInfo("No active scripts configured for auto-start.");
	}
}
		...
		if (!plugin.dynamicScriptCommands.has(commandId)) {
			...
			const command = plugin.addCommand({
				...
				callback: () => {
					...
					if (isActive) {
						plugin.logInfo(
							`Executing script via command: ${relativePath}`,
						);
						runPythonScript(plugin, scriptAbsolutePath, "manual"); // Don't await
					} else {
						plugin.logInfo(
