...
import {
	PythonBridgeSettings,
	JsonResponse,
	JsonRequest,
	ScriptSettingDefinition, // Keep if needed directly in main, otherwise remove
} from "./types";
import {
	DEFAULT_PORT,
	SETTINGS_DISCOVERY_TIMEOUT, // Keep if needed directly in main, otherwise remove
	PYTHON_LIBRARY_FILENAME, // Keep if needed directly in main, otherwise remove
} from "./constants";

// Import functions from new modules
import {
	checkPythonEnvironment,
	showPythonMissingNotification, // Keep if used directly, e.g., in catch blocks
	showRequestsMissingNotification, // Keep if used directly
} from "./environment_checker";
import {
	getScriptsFolderPath,
	updateAndSyncCommands,
	runPythonScript, // Keep for direct calls if any, or remove
	runAutoStartScripts,
	chooseAndRunPythonScript, // Keep for command callback
	runAllPythonScripts, // Keep for command callback
} from "./python_executor";
import {
	registerObsidianEventListeners,
	triggerEvent, // Keep if used directly, otherwise remove
} from "./event_handler";
...
export default class ObsidianPythonBridge extends Plugin {
	...
	async onload() {
		...
		// Validate startup folder path setting
		...
		if (envCheckOk) {
			...
			if (scriptsFolder && this.pythonExecutable) {
				...
				this.logWarn(
					"Skipping initial script settings discovery: Python executable or valid scripts folder not found.",
				);
			}
		} else {
			this.logWarn(
				"Skipping server start and settings discovery due to Python environment issues.",
			);
		}
	...
	async saveSettings() {
		...
		const actualListeningPort = this.server
		...
		if (this.server && currentPortSetting !== actualListeningPort) {
			if (
				...
				this.logError(
					`Attempted to save invalid port ${currentPortSetting}. Server not restarted.`,
				);
			}
		}
	...
	async validateScriptsFolderPathSetting(): Promise<void> {
		...
		if (!resolvedPath) {
			...
			this.logDebug(
				`Configured scripts folder path validated successfully on startup: ${resolvedPath}`,
			);
		}
	}
	...
	addCommands() {
		...
		this.addCommand({
			...
			callback: () => chooseAndRunPythonScript(this), // Use helper from python_executor
		});

		this.addCommand({
			id: "run-all-python-scripts",
			name: t("CMD_RUN_ALL_SCRIPTS_NAME"),
			callback: () => runAllPythonScripts(this), // Use helper from python_executor
		});

		this.addCommand({
			id: "refresh-script-settings",
			name: t("CMD_REFRESH_SCRIPT_SETTINGS_NAME"),
			callback: async () => {
				...
				try {
					// Use the combined update function from python_executor
					await updateAndSyncCommands(this, scriptsFolder);
	...
	startHttpServer() {
		...
		// Validate port
		if (
			!this.settings.httpPort && this.settings.httpPort !== 0 ||
			typeof this.settings.httpPort !== "number" ||
			!Number.isInteger(this.settings.httpPort) ||
			this.settings.httpPort < 0 ||
			this.settings.httpPort > 65535
			...
			async (req: http.IncomingMessage, res: http.ServerResponse) => {
				...
				req.on("end", async () => {
					...
					if (!res.writableEnded) {
						...
						this.logDebug(
							`HTTP Response sent (Status ${statusCode}).`,
						);
					}
				});
	...
	async handleAction(request: JsonRequest): Promise<JsonResponse> {
		// Simply delegate to the imported dispatchAction function
	...
	async requestUserInput(
		...
		return new Promise((resolve) => {
			...
			modal.onClose = () => {
				// Check if the promise is still pending (i.e., onSubmit wasn't called)
				// This requires a way to track if onSubmit was called, or rely on default behavior.
				// For simplicity, assume UserInputModal calls onSubmit *before* closing on submit.
				// If closed via Esc/button without submit, onSubmit isn't called, promise remains pending.
				// To resolve with null on cancel, UserInputModal needs modification or we add logic here.
				// Let's assume for now the Python side handles the timeout/error if no response comes.
				// Or modify UserInputModal to resolve(null) in onClose if not submitted.
				// --> For now, let's keep it simple: only resolves on submit. Cancellation leads to timeout on Python side.
			};
			modal.open();
		});
	}
