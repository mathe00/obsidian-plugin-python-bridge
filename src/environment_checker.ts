...
export async function checkPythonEnvironment(
	...
	if (!yamlInstalled) {
		plugin.logWarn(
			`Optional Python module 'PyYAML' not found using ${pythonCmd}. Frontmatter property management features will not work.`,
		);
...
export async function findPythonExecutable(
	...
	for (const cmd of commandsToTry) {
		try {
			...
			await new Promise<void>((resolve, reject) => {
				...
				process.on("close", (code) => {
					if (isWindows && code === 9009) {
						...
						reject(
							new Error(
								`Command ${cmd} --version exited with code ${code}`,
							),
						);
					}
				});
			...
			plugin.logDebug(
				`Command '${cmd}' not found or failed version check: ${error instanceof Error ? error.message : String(error)}`,
			);
		}
	}
 ...
 */
export async function checkPythonModule(
	plugin: ObsidianPythonBridge,
	pythonCmd: string,
	moduleName: string,
): Promise<boolean> {
	...
	try {
		await new Promise<void>((resolve, reject) => {
			...
			process.on("close", (code) => {
				if (code === 0) {
					...
					plugin.logWarn(
						`Failed to import module '${moduleName}' using '${pythonCmd}'. Exit code: ${code}.`,
					);
					if (stderrOutput.trim()) {
						plugin.logWarn(
							`Stderr from failed import: ${stderrOutput.trim()}`,
						);
					}
					reject(
						new Error(
		...
		plugin.logDebug(
			`Module check failed for '${moduleName}' using '${pythonCmd}': ${error instanceof Error ? error.message : String(error)}`,
		);
 ...
 * @param plugin The ObsidianPythonBridge plugin instance.
 */
export function showPythonMissingNotification(
	plugin: ObsidianPythonBridge,
): void {
 ...
 */
export function showRequestsMissingNotification(
	plugin: ObsidianPythonBridge,
	pythonCmd: string,
): void {
	// Delay notice slightly to ensure Obsidian UI is ready
