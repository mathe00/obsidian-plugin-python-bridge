...
export default {
  ...
  NOTICE_INVALID_PORT_CONFIG_SUFFIX:
    'サーバーは起動しませんでした。設定で有効なポート（1-65535）を設定してください。',
  ...
  SETTINGS_REFRESH_DEFINITIONS_BUTTON_DESC:
    'スクリプトフォルダを再スキャンして、Python スクリプト内で定義された設定を検出または更新します。',
  ...
  SETTINGS_SCRIPT_FOLDER_NOT_CONFIGURED:
    'Python スクリプトフォルダが設定されていません。上でパスを設定してください。',
  SETTINGS_NO_SCRIPT_SETTINGS_FOUND:
    '設定されたフォルダに定義可能な設定を持つスクリプトが見つからないか、設定の検出に失敗しました。「定義を更新」をクリックして再試行してください。',
  ...
  NOTICE_PYTHON_EXEC_MISSING_FOR_RUN:
    'スクリプトを実行できません：Python 実行可能ファイルが見つかりません。インストールと PATH を確認してください。',
  CMD_REFRESH_SCRIPT_SETTINGS_NAME: 'Python スクリプト設定定義を更新',
  SETTINGS_SECURITY_WARNING_TITLE: 'セキュリティ警告',
  SETTINGS_SECURITY_WARNING_TEXT:
    '任意の Python スクリプトの実行にはリスクが伴う可能性があります。実行するスクリプトのソースを信頼できることを確認してください。スクリプトはシステムやデータにアクセスする可能性があります。プラグインの作者およびスクリプトの作者は、ユーザーが実行を選択したスクリプトによって引き起こされたデータの損失やセキュリティの問題について責任を負いません。自己責任でスクリプトを実行してください。',
  ...
  SETTINGS_SCRIPT_AUTOSTART_DELAY_DESC:
    'Obsidianの起動後、スクリプトを実行する前にこの秒数待機します（「起動時に実行」がオンの場合のみ適用）。遅延なしの場合は0を使用します。',

  SETTINGS_AUTO_PYTHONPATH_NAME: 'ライブラリのPYTHONPATHを自動設定',
  SETTINGS_AUTO_PYTHONPATH_DESC:
    'スクリプト実行時にプラグインディレクトリを自動的にPYTHONPATHに追加し、Pythonライブラリの直接インポートを可能にします（推奨）。無効にした場合、ObsidianPluginDevPythonToJS.pyをスクリプトフォルダにコピーするか、sys.pathを手動で管理する必要があります。',
  NOTICE_AUTO_PYTHONPATH_DISABLED_DESC:
    '自動PYTHONPATHが無効になりました。ObsidianPluginDevPythonToJS.pyがスクリプトフォルダにあることを確認するか、sys.pathを手動で管理してください。',
