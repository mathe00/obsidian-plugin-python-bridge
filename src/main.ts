...
import { PythonBridgeSettings, JsonResponse, JsonRequest } from './types'; // Keep if needed directly in main, otherwise remove
import { DEFAULT_PORT } from './constants'; // Keep if needed directly in main, otherwise remove

// Import functions from new modules
import { checkPythonEnvironment } from './environment_checker'; // Keep if used directly, e.g., in catch blocks
import {
  ...
  runAllPythonScripts,
} from './python_executor'; // Keep for direct calls if any, or remove // Keep for command callback // Keep for command callback
import { registerObsidianEventListeners } from './event_handler'; // Keep if used directly, otherwise remove
...
export default class ObsidianPythonBridge extends Plugin {
  ...
  async onload() {
    ...
    if (adapter instanceof FileSystemAdapter && this.manifest.dir) {
      ...
      this.logDebug(
        `Determining plugin path from basePath: '${basePath}' and manifestDir: '${manifestDir}'`
      );
      const joinedPath = path.join(basePath, manifestDir);
      ...
      if (basePath.startsWith('/') && !this.pluginDirAbsPath.startsWith('/')) {
        this.logWarn(
          `normalizePath might have removed the leading slash. Original joined: ${joinedPath}, Normalized: ${this.pluginDirAbsPath}. Attempting to prepend slash.`
        );
  ...
  async loadSettings() {
    ...
    if (
      ...
      (this.settings.httpPort !== 0 &&
        (this.settings.httpPort < 1024 || this.settings.httpPort > 65535))
    ) {
      this.logWarn(
  ...
  addCommands() {
    ...
    this.addCommand({
      ...
      callback: () => chooseAndRunPythonScript(this),
    }); // Use helper from python_executor
    this.addCommand({
      id: 'run-all-python-scripts',
      name: t('CMD_RUN_ALL_SCRIPTS_NAME'),
      callback: () => runAllPythonScripts(this),
    }); // Use helper from python_executor
    this.addCommand({
  ...
  async requestUserInput(
    ...
    return new Promise((resolve) => {
      ...
      modal.onClose = () => {
        /* Assume Python side handles timeout/error if no response comes */
      };
      modal.open();
    });
  }
