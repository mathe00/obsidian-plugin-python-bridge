...
import {
	...
	deletePath,
	renamePath,
	runObsidianCommand, // Keep import even if disabled for now
	getAllTags, // Keep import even if disabled for now
	getVaultName,
	getThemeMode,
	createFolder,
	listFolder,
	...
	getEditorContext,
	getBacklinks,
	modifyNoteContentByRelativePath, // Use the relative path version
} from "./obsidian_api";
	...
	try {
		switch (action) {
			...
			case "get_all_note_paths":
				// Directly call the function from obsidian_api.ts
				return {
					status: "success",
					data: plugin.getAllNotePaths(), // Use plugin method which calls obsidian_api internally
				};
			...
			case "get_note_content":
				...
				try {
					const content = await getNoteContentByPath(
						plugin,
						payload.path,
					);
			...
			case "show_notification":
				...
				const duration =
					typeof payload?.duration === "number"
						? payload.duration
						: 4000;
				// Use plugin's method directly for Notice
				plugin.showNotification(payload.content, duration);
				return { status: "success", data: null };
			...
			case "get_script_settings":
				...
				plugin.logDebug(
					`Requesting settings for script: ${relativePath}`,
				);
			...
			case "rename_path":
				if (
					typeof payload?.old_path !== "string" ||
					!payload.old_path
				) {
					return {
						status: "error",
				...
				if (
					typeof payload?.new_path !== "string" ||
					!payload.new_path
				) {
					return {
						status: "error",
			...
			case "get_backlinks":
				...
				const cacheMode =
					payload.cache_mode === "safe" ? "safe" : "fast";
				try {
